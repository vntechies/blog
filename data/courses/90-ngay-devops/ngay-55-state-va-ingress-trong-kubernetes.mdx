---
title: 'Ngày 55 - State và Ingress trong Kubernetes'
images: ['/static/images/ogps/90daysofdevopsd55.png']
summary: 'Trong bài viết cuối cùng về Kubernetes, chúng ta sẽ xem xét State và Ingress và các ứng dụng có trạng thái'
...
Trong bài viết cuối cùng về Kubernetes, chúng ta sẽ xem xét State và Ingress.
...
Một bản tóm tắt về các lệnh minikube mà chúng ta sẽ sử dụng để có các chứ năng và các addons để sử dụng persistence là
...
![](/static/images/assets/Day55_Kubernetes1.png)

Bạn có thể tìm thấy tệp cấu hình YAML cho ứng dụng này tại [pacman-stateful-demo.yaml](https://github.com/vntechies/90DaysOfDevOps/tree/main/2022/Days/Kubernetes/pacman-stateful-demo.yaml)
...
![](/static/images/assets/Day55_Kubernetes4.png)

Giờ đây chúng ta đã có namespace mới được tạo.

![](/static/images/assets/Day55_Kubernetes5.png)
...
![](/static/images/assets/Day55_Kubernetes6.png)
...
![](/static/images/assets/Day55_Kubernetes7.png)
...
![](/static/images/assets/Day55_Kubernetes8.png)

Bây giờ chúng ta có thể chơi và ghi điểm số, sau đó nó sẽ được lưu trong cơ sở dữ liệu của chúng ta.

![](/static/images/assets/Day55_Kubernetes9.png)

OK, thât tuyệt nếu chúng ta có thể có một điểm số cao nhưng điều gì sẽ xảy ra nếu chúng ta xoá pod `mongo-0`? Bằng lệnh

```shell
kubectl delete pod mongo-0 -n pacman
```

tôi có thể xoá nó và nếu bạn vẫn ở trong ứng dụng, bạn sẽ thấy điểm số sẽ không khả dụng ít nhất trong vài giây.

![](/static/images/assets/Day55_Kubernetes10.png)
...
![](/static/images/assets/Day55_Kubernetes11.png)

Với việc triển khai, chúng ta có thể mở rộng quy mô ứng dụng này bằng cách sử dụng các lệnh trong phần trước nhưng nếu bạn muốn tổ chức một bữa tiệc Pacman siêu to khổng lồ, bạn có thể mở rộng quy mô bằng cách sử dụng lệnh

```shell
kubectl scale deployment pacman --replicas=10 -n pacman
```

![](/static/images/assets/Day55_Kubernetes12.png)
...
![](/static/images/assets/Day55_Kubernetes13.png)
...
![](/static/images/assets/Day55_Kubernetes14.png)

Bây giờ, chúng ta phải tạo cấu hình YAML cho ingress để truy cập dịch vụ pacman. Tôi đã thêm tệp này vào repository này [pacman-ingress.yaml](https://github.com/vntechies/90DaysOfDevOps/tree/main/2022/Days/Kubernetes/pacman-ingress.yaml)

Sau đó, chúng ta có thể tạo nó trong ingress namespace của chúng ta với lệnh

```shell
kubectl create -f Pacman-ingress.yaml
```

![](/static/images/assets/Day55_Kubernetes15.png)

Sau đó, nếu chúng ta chạy `kubectl get ingress -n Pacman`

![](/static/images/assets/Day55_Kubernetes16.png)

Sau đó, tôi được thông báo rằng vì chúng ta đang sử dụng minikube chạy trên WSL2 trong Windows nên chúng ta phải tạo minikube tunnel bằng cách sử dụng lệnh
...
Hẹn gặp lại vào [ngày 56](https://vntechies.dev/courses/90-ngay-devops/ngay-56-buc-tranh-toan-canh-co-so-ha-tang-duoi-dang-ma-iac).
