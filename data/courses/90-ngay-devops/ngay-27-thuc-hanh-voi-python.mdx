---
title: 'Ngày 27 - Thực hành với Python'
images: ['/static/images/ogps/90daysofdevopsd27.png']
summary: 'Trong phần cuối của loạt bài về mạng máy tính, chúng ta sẽ tìm hiểu một số tác vụ và công cụ tự động hóa dựa trên môi trường lab đã được tạo ra'
...
Trong phần cuối của loạt bài về mạng máy tính, chúng ta sẽ tìm hiểu một số tác vụ và công cụ tự động hóa dựa trên môi trường lab đã được tạo ra trong [Ngày 26](https://vntechies.dev/courses/90-ngay-devops/ngay-26-xay-dung-lab).

Chúng ta sẽ sử dụng SSH để kết nối đến các thiết bị trong mạng. Giao tiếp dựa trên SSH sẽ được mã hóa như đã giới thiệu trước đây trong loạt bài về hệ điều hành Linux. Xem lại [ngày 18](https://vntechies.dev/courses/90-ngay-devops/ngay-18-ssh-va-may-chu-web-lamp-stack).
...
![](/static/images/assets/Day27_Networking3.png)
...
![](/static/images/assets/Day27_Networking4.png)
...
![](/static/images/assets/Day27_Networking5.png)

### Sử dụng Python để thu thập thông tin từ các thiết bị

Ví dụ đầu tiên là sử dụng Python để thu thập thông tin từ tất cả các thiết bị của mình. Cụ thể hơn, tôi sẽ kết nối đến từng thiết bị và chạy một lệnh đơn giản để lấy thông tin cấu hình của mỗi card mạng. Tôi đã lưu chương trình này tại đây [netmiko_con_multi.py](https://github.com/vntechies/90DaysOfDevOps/tree/main/2022/Days/Networking/netmiko_con_multi.py)

Khi tôi chạy chương trình này, tôi có thể thấy cấu hình của mỗi cổng trên tất cả các thiết bị của mình.

![](/static/images/assets/Day27_Networking6.png)
...
Chúng ta có thể sử dụng chương trình [netmiko_sendchange.py](https://github.com/vntechies/90DaysOfDevOps/tree/main/2022/Days/Networking/netmiko_sendchange.py) để thực hiện điều này. Thao tác này sẽ kết nối qua SSH và thực hiện thay đổi cần thiết trên `SW1` và `SW2`.

![](/static/images/assets/Day27_Networking7.png)

Nếu bạn đã xem code, bạn sẽ thấy thông báo xuất hiện và cho chúng ta biết `sending configuration to device` nhưng không có xác nhận rằng điều này đã được thực hiện, chúng ta có thể thêm đoạn code bổ sung vào chương trình để thực hiện kiểm tra và xác thực việc cấu hình trên các switch hoặc chúng ta có thể sửa đổi đoạn code của ví dụ thứ nhất để cho chúng ta thấy điều đó. [netmiko_con_multi_vlan.py](https://github.com/vntechies/90DaysOfDevOps/tree/main/2022/Days/Networking/netmiko_con_multi_vlan.py)

![](/static/images/assets/Day27_Networking8.png)

### Sao lưu cấu hình của các thiết bị

Một ví dụ khác là sao lưu các cấu hình mạng của các thiết bị. Nếu bạn không muốn kết nối với mọi thiết bị có trên mạng của mình, bạn có thể chỉ định thiết bị mà bạn muốn sao lưu. Bạn có thể tự động hóa việc này bằng cách sử dụng chương trình [backup.py](https://github.com/vntechies/90DaysOfDevOps/tree/main/2022/Days/Networking/backup.py). Bạn sẽ cần điền vào file [backup.txt](https://github.com/vntechies/90DaysOfDevOps/tree/main/2022/Days/Networking/backup.txt) các địa chỉ IP mà bạn muốn sao lưu.

Chạy chương trình trên và bạn sẽ thấy nội dung như bên dưới.

![](/static/images/assets/Day27_Networking9.png)

Đây chỉ là vài thông tin đơn giản được in ra màn hình, tôi sẽ cho bạn xem các file sao lưu.

![](/static/images/assets/Day27_Networking10.png)
...
Hẹn gặp lại các bạn vào [Ngày 28](https://vntechies.dev/courses/90-ngay-devops/ngay-28-buc-tranh-toan-canh-devops-va-cloud), nơi mà chúng ta sẽ tìm hiểu về điện toán đám mây (cloud computing) và các kiến thức cơ bản xoay quanh chủ đề này.
