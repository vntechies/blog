...
Hôm qua, chúng ta đã hoàn thành mã giống như file [day11_example4.go](https://github.com/vntechies/90DaysOfDevOps/blob/main/Days/Go/day11_example4.go), chúng ta đã xác định thủ công các biến và hằng số `challenge, daystotal, dayscomplete`.

Bây giờ hãy thêm một biến mới có tên là `TwitterName`, bạn có thể tìm thấy mã mới này tại [day12_example1.go](https://github.com/vntechies/90DaysOfDevOps/blob/main/Days/Go/day12_example1.go) và đây sẽ là đầu ra nếu chạy mã này.
...
Trong mã của chúng ta [day12_example2.go](https://github.com/vntechies/90DaysOfDevOps/blob/main/Days/Go/day12_example2.go), bạn có thể thấy rằng chúng ta yêu cầu người dùng nhập hai biến, `TwitterName` và` DaysCompleted`
...
Bạn có thể xem chương trình đã hoàn thành tại đây [day12_example3.go](https://github.com/vntechies/90DaysOfDevOps/blob/main/Days/Go/day12_example3.go).
...
Hãy đơn giản hóa mã và xem chương trình thay đổi khi có và không có `&` trước một các lệnh in của chúng ta, nó sẽ in ra địa chỉ bộ nhớ của con trỏ. Tôi đã thêm ví dụ ở đây. [Day12_example4.go](https://github.com/vntechies/90DaysOfDevOps/blob/main/Days/Go/day12_example4.go)
