...
Chúng ta sẽ sử dụng các icon sau để biểu thị một vài danh mục

- 💩 - mã mà bạn nên tránh.
- 🤔 - mã nói chung là ổn, nhưng bỏ qua các tính năng được tích hợp trong ngôn ngứ
- 🐍 - mã được coi là tốt.
- 💡 - bonus.
- ❌ - điểm chưa tốt, nên tránh
- ✅ - điểm tốt, nên dùng

## 1. Kiểm tra null

Một trường hợp rất hay xảy ra là kiểm tra xem một biến có trống không (hoặc null) trước khi sử dụng. Trong Python, _null_ được biểu thị bằng từ khóa `None`. Hai đoạn mã bên dưới tạo ra kết quả giống hệt nhau. sử dụng từ khóa `if` theo sau là tên biến.

💡 _Mẹo bổ sung_: Sử dụng `f-strings` để định dạng chuỗi. Bắt đầu chuỗi f bằng cách nhập _f_ ngay trước chuỗi thông thường `(f"...")` và đặt các biến bên trong dấu ngoặc nhọn, Python sẽ định dạng kết quả thành một chuỗi chứa các biến.
...
# Cách tốt hơn 🐍: Sử dụng enumerate nếu bạn cần dùng cặp index, valu ✅
