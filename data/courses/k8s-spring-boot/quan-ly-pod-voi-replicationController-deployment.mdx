...
- Thực hiện thay đổi podTemplate
  ...
  Khi chúng ta thực hiện thay đổi podTemplate và triển khai lại Deployment thì thấy có replicaSet `k8s-springboot-series-87d545bb5` được tạo mới, replicaSet `k8s-springboot-series-7cbfd878f6` đã giảm số lượng pod về 0. Khi thay đổi podTemplate thì deployment sẽ tạo một replicaSet mới. Các pod (phiên bản mới) sẽ thay thế dần dần các pod (phiên bản cũ) cho đến khi các pod (phiên bản cũ) đã được thay thế hết bằng các pod (phiên bản mới). Cách mà Deployment cập nhật phiên bản của ứng dụng đó gọi các các chiến lược deployment (Deployment Strategy), Deployment sử dụng RollingUpdate là chiến lược mặc định.

- Thực hiện rollback giữa các phiên bản của ứng dụng.

  Khi thực hiện cập nhật podTemplate thấy rằng sẽ tạo replicaSet mới và replicaSet cũ thì không bị xóa đi mà chỉ giảm số lượng pod về 0. Tại sao các replicaSet trước đó không bị xóa đi nhỉ? Giả sử khi cập nhật phiên bản mới cho ứng dụng nhưng phiên bản mới này có vấn đề và bạn muốn quay về lại phiên bản trước đó của ứng dụng thì việc giữ lại các replicaSet trước sẽ giúp thay đổi giữa phiên bản của ứng dụng.

  ```shell
  # 1. Lấy danh sách version của ứng dụng.
  ...
  kubectl annotate deployment/<DEPLOYMENT_NAME> kubernetes.io/change-cause="thêm note ở đây"
  ...
  # Lấy lại danh sách phiên bản của Re
  kubectl rollout history deployment k8s-springboot-series
  deployment.apps/k8s-springboot-series
  REVISION  CHANGE-CAUSE
  1         <none>
  ...
  # Thực hiện rollback về phiên bản cụ thể nào đó (từ revision: "2" => revision: "1")
