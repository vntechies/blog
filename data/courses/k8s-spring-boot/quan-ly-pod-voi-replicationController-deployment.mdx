...
Thay vì định nghĩa và triển khai pod một cách trực tiếp thì sẽ dùng `Replication Controller` để tạo và quản lý pod nhằm đảm bảo số lượng pod chạy trên cùng một thời điểm. Như vấn đề ở phần 3.1 thì mong muốn khi pod nào đó bị crash hoặc bị xóa thì sẽ có các pod mới được tạo lên tránh việc request gọi vào ứng dụng bị lỗi.

**3. Tạo Replication Controller bằng file YAML**
