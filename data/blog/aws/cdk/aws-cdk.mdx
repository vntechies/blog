...
tags: ['aws', 'aws cdk']
draft: false
images: ['/static/images/ogps/aws-sdk.png']
summary: 'AWS CDK, những khái niệm cơ bản, làm sao để bắt đầu, những good practices và tools để làm việc hiệu quả hơn'
---

![AWS Cloud Development Kit](/static/images/ogps/aws-sdk.png)
...
Phiên bản beto công khai đầu tiên của AWS CDK (v.0.8.0) được công bố vào năm 2018, lúc đó CDK như một cách tiếp cận thay thế cho việc triển khai các AWS resources bằng code sử dụng ngôn ngữ Typescript.

AWS CDK là một [open source project](https://github.com/aws/aws-cdk) và bản Generally Available (GA) của AWS CDKsv2 đã được Werner Vogels - CTO của Amazon công bố vào tháng 12 năm 2021 trong sự kiện re:invent.
...
Nói đơn giản thì AWS CDK triển khai hệ thống của bạn từ 3 thành phần chính:

- App: Là thành phần chính của cấu trúc, hợp nhất tất cả các stacks trong ứng dụng và sau đó triển khai trên AWS
- Stack: gần tương tự với Stack trong CloudFormation, là một đơn vị (template) chứa các tài nguyên AWS ở dạng cấu trúc (constructs) và có thể sử dụng để triển khai.
- Constuct: là thành phần cơ bản chứa các tài nguyên AWS (một hoặc nhiều). Bạn có thể tuỳ ý tạo ra và kết hợp các tài nguyên để tạo ra cấu trúc (construct) riêng của mình

![AWS CDK Diagram](/static/images/assets/aws-cdk-diagram.png)

Khi một ứng dụng AWS được tổng hợp, một CloudFormation template sẽ được tạo ra chứa toàn bộ các thành phần của hệ thống.
Template này có thể được triển khai lên AWS bằng AWS CDK CLI hoặc CloudFormation.
...
Ví dụ: bạn có thể nhóm nhiều tài nguyên lại với nhau để tạo thành 3 tiers web application, EC2 instances cho backend + frontent được liên kết với RDS và cuối cùng là Application Loadbalancer để cân bằng tải.
...
- Đóng góp, phản hồi trực tiếp vợi đội phát triển vì nó open source 😌
- Khả năng tạo các template CloudFormation lớn với một vài dòng code
- Sử dụng vòng lặp để tạo nhiều tài nguyên giống nhau thay vì viết đủ các dòng code (ví dụ: tạo 5 hàm lambda giống nhau mà không cần viết lại code 5 lần)
...
Hầu hết tài liệu tìm được là ở trên các trang hướng dẫn của AWS, nhưng AWS CDK thì rất khác.
Ngoài điều này, AWS CDK còn một số điểm khác biệt so với CloudFormation:

- Sử dụng L2/L3 Construct sử dụng các cài đặt tốt nhất theo các best pracites khiến việc phát triển nhanh chóng và an toàn thay vì phải áp dụng từng setting cho từng tài nguyên như ở CloudFormation.
- Sử dụng ngôn ngữ lập trình thay vì sử dụng "ngôn ngữ" YAML hoặc JSON. Thời gian sử dụng ngôn ngữ lập trình có ích cho sự nghiệp hơn là viết YAML code cho CloudFormation
...
<blockquote class="twitter-tweet">
  ...
  <a href="https://twitter.com/ben11kehoe/status/1466465582501531662?ref_src=twsrc%5Etfw">
    December 2, 2021
  </a>
</blockquote>

Dù sao thì, AWS CDK chỉ là một cách tiếp cận khác và cuối cùng vẫn tổng hợp thành một AWS CloudFormation template.
Chính vì vậy, bạn vẫn nên sử dụng các tool như [cfn-lint](https://www.google.com/search?q=cfn-lint&oq=cfn-lint&aqs=chrome..69i57.101j0j1&sourceid=chrome&ie=UTF-8) and [checkov](https://www.checkov.io/)
để ngăn chặn việc cấu hình sai cho hệ thống của bạn.

Việc có một công cụ abstract CloudFormation Như AWS CDK sẽ rất tốt nếu bạn thực sự hiểu những gì diễn ra ở phía sau nó. Nếu bạn không hiểu CloudFormation làm như thể nào để triển khai các tài nguyên, bạn có thể mất nhiều thời gian hơn trong việc gỡ lỗi và không thể lý giải tại sao AWS CDK app của mình không hoạt động.
...
- Sử dụng `cdk diff` để kiểm tra/review các thay đổi trước khi triển kahi trên production.
- Phân tách stateful constrcuts chứa các tài nguyên như database, storage riêng biệt với các stateless constructs chứa các tài nguyên như API, ECS, Monitoring,... Từ đó bạn có thể bật chế độ bảo vệ (termination protection) cho các tài nguyên stateful và có thể an toàn xoá, tạo mới các stack chứa các tài nguyên stateless mà không cần lo việc mất dữ liệu.
...
Terraform Sử dụng ngôn ngữ, syntax riêng cho việc viết code (HCL) còn với AWS CDK, bạn có thể lựa chọn ngôn ngữ mà bạn thấy phù hợp để làm việc cùng. Trong một diễn biến mới [Terraform cũng mới ra mắt CDK của riêng mình](https://www.terraform.io/cdktf)
...
Nếu bạn muốn tìm hiểu sâu hơn, hãy tham khảo các [ví dụ của AWS CDK](https://github.com/aws-samples/aws-cdk-examples) và bắt đầu xây dựng các ứng dụng thực tế sử dụng AWS CDK.
