...
## 4. Tổng kết or Kết luận

Trong bài mở đầu chuỗi( hoặc series vì 2 từ có nghĩa giống nhau) triển khai  ứng dụng Spring boot với Kubernetes này đã giới thiệu về mục tiêu của series, các bước chuẩn bị về môi trường cũng như giới thiệu qua về spring boot application mà sẽ sử dụng. Ở bài tiếp theo thì chúng ta sẽ đi tìm hiểu về cách đóng gói spring boot container với `Pod` trong Kubernetes.
