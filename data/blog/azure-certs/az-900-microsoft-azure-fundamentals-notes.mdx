...
- Application GW is a high level of load balancer
- Works on HTTP request of the traffic instead of the IP address and port
- Traffic on specific web address go to a specific machine
- Fit for most Azure services

### Content delivery network (CDK)

- Distributed network of servers that can deliver web content via the edge node which is closest to the user.
...
- Storage account must be unique accross global (same to AWS S3) as every object has its own web address
...
3. Page:
   ...
   - Any part of the file could be accessed at any time (ex: virtual hard drive)
...
- Constraints -> Contains limited amount of storage
- Backups -> time and resources spent on maintaining backups
- Security -> difficult to keep all data secure at all times, Specialist assistance is often needed
...
- Easy synchronization -> Traditional databases that aren't cloud enabled could be very difficult to set up across multiple regions
...
- Scalability:
  ...
  2. Infinity resource -> Any number of users of your application can be supported
  3. Lowest price -> Even through the scaling is automatic, only pay for the resources that are used
- Deverloper: various SDKs, APIs and supported languages: C#, Java, Python, Nodejs,...
- Paltforms: Choose from numerous data platforms such as SQL, MongoDB, and Cassandra

\*_HOWEVER, COST CAN RUN UP FAST_
